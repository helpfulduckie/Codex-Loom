...
- **`budget: { anchor: 800, standard: 400, minor: 200 }`** — reads the card's role from
  ...
  cap is `CL0712`'s job.
- **`count: { default: { max: 5 }, fields: { vibe: { min: 3, max: 5 }, tagline: { words: { min: 3, max: 5 } } } }`**
...
| `CL-duckieConv/0002` | List-length caps — `vibe` 3–5, `personality.keywords` 2–4, `tagline` 3–5 words, `*` default of at most 5. Inline only |
