...
const REGISTRY = Object.freeze({
  ...
  FIELD_TABLE_UNUSABLE:         { id: 'CL0422', severity: ERROR, summary: 'A field table or templateFor slot file cannot be read as a mapping, so its fields, groups and templates are unavailable; repair the file.' },
  ...
  LEAKED_FIELD_TOKEN:           { id: 'CL0430', severity: ERROR, summary: 'Compiled output still contains a field, pronoun or character token, so AID will receive the literal token and the build fails; fix the authoring reference or resolver input.' },
  LEAKED_VARIABLE:              { id: 'CL0431', severity: ERROR, summary: 'Compiled output still contains a compile.yaml variable token, so AID will receive the literal token and the build fails; declare or correct the variable reference.' },
  LEAKED_RENDER_FUNCTION:       { id: 'CL0432', severity: ERROR, summary: 'Compiled output still contains a render-function call, so AID will receive unevaluated syntax and the build fails; fix the source call or renderer.' },
  ...
  LEAKED_JS_ARTIFACT:           { id: 'CL0435', severity: ERROR, summary: 'Compiled output contains a JavaScript interpolation artifact, so AID will receive corrupted text and the build fails; fix the source interpolation.' },
  SUSPECT_VERB_MARKER:          { id: 'CL0436', severity: WARN,  layer: 'opinion', summary: 'Compiled output contains an unrecognized bracketed lowercase word, so the intended conjugation may be wrong; correct the source marker if it is a typo.' },
  SUSPECT_JS_WORD:              { id: 'CL0437', severity: WARN,  layer: 'opinion', summary: 'Compiled output contains bare undefined or NaN, so authoring data or interpolation may be missing; fix the source value or interpolation.' },
