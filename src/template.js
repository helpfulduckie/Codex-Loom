      ...
      if (inner.startsWith(prefix)) {
        try {
          ...
          options.diagnostics.error(
            ...
            `render function in field value: ${e.message}; correct the call syntax, and the malformed call remains literal.`,
            { file: options.file },
          );
          return match;
        }
...
function expandIncludes(source, partials, report, variables, diagnostics, file, stack) {
  ...
  return source.replace(/\{include\s+(\S+)\}/g, function(match, includeName, offset, whole) {
    ...
    if (stack.includes(key)) {
      report(CODES.PARTIAL_CYCLE, `Circular partial include: ${[...stack, key].join(' → ')}; break the include cycle, and the cyclic directive renders empty.`, { line });
      return '';
    }
    const partial = partials.get(key);
    if (!partial) {
      report(CODES.PARTIAL_NOT_FOUND, `Unknown partial "${includeName}" (no .partial file found); add or rename the partial, and the directive renders empty.`, { line });
      return '';
    }
